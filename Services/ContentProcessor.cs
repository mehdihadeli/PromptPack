...
public class ContentProcessor(ILogger<ContentProcessor> logger) : IContentProcessor
{
    public async Task<ProcessedFile> ProcessFileAsync(
        string filePath, string baseDirectory, bool removeComments, bool compress)
    {
        ...
        foreach (var line in lines)
        {
            ...
            if (inMultiLineComment)
            {
                if (trimmed.Contains("*/"))
                {
                    ...
                    var remaining = trimmed[(trimmed.IndexOf("*/", StringComparison.Ordinal) + 2)..];
    ...
    private string CompressContent(string content, string extension)
    {
        ...
        foreach (var line in lines)
        {
            ...
            if (trimmed.Contains("class ") || trimmed.Contains("interface ") || 
                ...
                trimmed.Contains("protected ") || trimmed.Contains("import ") || 
